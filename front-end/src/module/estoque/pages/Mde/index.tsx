...
function Mde() {
  ...
  const renderCell = (element: any) => {
    if (element.value === "M") {
      ...
      return (
        <span
          className="font-bold text-white"
          style={{ color: theme.colors.error, fontSize: "12px" }}
        >
          A MANIFESTAR
        </span>
      );
      ...
      return (
        <span
          className="font-bold text-white"
          style={{ color: theme.colors.warning, fontSize: "12px" }}
        >
          AVULSA
        </span>
      );
  ...
  const data = [
    {
      numero: "12368498",
      emissao: "11/05/2022",
      cnpj: "03.406.025/0001-35",
      fornecedor: "CADA",
      vlrbruto: 1502,
      vlricms: 12,
      vlrdesconto: 32,
      vlrliquido: 143269588,
      status: "A",
      manifesto: "12/05/2022",
      entrada: "",
      incluida: "N",
    },
    {
    ...
    if (desc !== "") {
      ...
      if (!isNaN(parseFloat(desc)) && isFinite(desc)) {
        notas = dataSourceCopy.filter((produto) => {
          return produto.numero.includes(desc);
        });
      } else {
        notas = dataSourceCopy.filter((produto) => {
  ...
  return (
    <Container
      ...
      style={{
        backgroundColor:
          theme.title === "dark" ? theme.colors.tertiary : theme.colors.white,
      }}
    >
      <header
        className="flex text-xl font-bold items-center justify-between mb-1 h-9"
        style={{ color: theme.colors.primary }}
      >
        <div
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          {/* <ButtonIcon className="text-sm" label="" icon={<FaFunnelDollar />} width='100%' style={{ height: '28px' }} onClick={() => setShowModalFiltro(true)} /> */}
        </div>
      </header>
      <hr className="mb-5" />
      <Body>
        ...
        <ButtonIcon className="mr-3" label="Manifestar nota" icon={<FaFileSignature />} width={'19%'} style={{ marginTop: '-3px' }} color={theme.colors.warning} onClick={onManifestar} /> */}
          <div className="flex w-20">
            <div
              className="linha-vertical h-8 m-2"
              style={{ marginTop: "-1px" }}
            ></div>
          ...
          <div className="text-xs flex w-50">
            <div
              className="linha-vertical h-8 m-2"
              style={{ marginTop: "-1px" }}
            ></div>
            <div className="text-left">
              <p className="font-bold">Última consulta </p>
          ...
          <div className="text-xs flex w-50">
            ...
            <div className="text-center">
              <p className="font-bold">Tempo restante </p>
              <span
                className=""
                style={{ color: theme.colors.error, fontSize: "15px" }}
              >
                09:06
              </span>
            </div>
