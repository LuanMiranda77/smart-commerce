  ...
  const data = [
    {
      ...
      descricao: "PRODUTO TESTE DA FAZENDA",
      un: "KG",
      quant: 999,
      unNota: "UND",
      quantNota: 999,
      ...
      if (!isNaN(parseFloat(desc)) && isFinite(desc)) {
        if (desc.length < 12) {
          notas = dataSourceCopy.filter((produto) => {
            return produto.codigo.includes(desc);
          });
        } else {
          notas = dataSourceCopy.filter((produto) => {
            return produto.codbarras.includes(desc);
          });
        }
      } else {
        notas = dataSourceCopy.filter((produto) => {
          return produto.descricao.includes(desc.toUpperCase());
        });
      }
      setDataSource(notas);
    } else {
    ...
    >
      <ContainerEntradaNota className="p-2 flex">
        ...
        <hr className="mb-2" style={{ border: '1px solid' + theme.colors.gray }} /> */}
          ...
          <InputDefault className=" mr-5 mb-3" label="Fornecedor" type="text" />
        ...
        <div
          className="linha-vertical h-full m-2"
          style={{ marginTop: "-1px", color: theme.colors.gray }}
        ></div>
          ...
          <ContainerProdutoSync>
            <DataGridDefault
              ...
              onSelectionChanged={(e) =>
                setProdutoSelect(e.selectedRowsData[0])
              }
            >
              <Column
                dataField="codigo"
